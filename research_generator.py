...
def generate_subcategories(topic):
    ...
    if response:
        ...
        print(f"Raw API response for subcategories: \n---\n{raw_response}\n---")
...
def generate_references(topic):
    """
    Generates a list of plausible, but fictional, academic sources for a given topic.
    """
    print(f"\n🧠 Generating plausible references for: '{topic}'...")

    prompt = f"""You are an expert academic librarian. For the topic '{topic}', generate a list of 5 to 7 plausible but fictional academic sources. These should include books, journal articles, and scholarly works. Format each reference as a complete, single-line entry. Do not include any other text, explanations, or titles like 'Books' or 'Journal Articles'. Just the list.
    """
    ...
    if response:
        raw_response = response.get("data", "").strip()
        references = [ref.strip() for ref in raw_response.split('\n') if ref.strip()]
        print(f"✅ References generated.")
        return references
    
    print("❌ Failed to generate references.")
    return []

# Updated research_subcategory function
def research_subcategory(subcategory):
    """
    Researches a subcategory by prompting the LLM for a summary.
    It returns a single string of body_text.
    """
    print(f"\nStep 2: Researching subcategory: '{subcategory}'")
    
    system_message = """You are an expert academic researcher. For a given subtopic, synthesize a detailed summary.
    Your summary must be in plain text and should not contain any in-text citations or references.
    """
    
    messages = [
        {"role": "user", "content": f"Synthesize information to research the subtopic: '{subcategory}'."}
    ]
    ...
    if response:
        ...
        return raw_response, [] # Returns a tuple to be consistent with the main loop
...
def generate_docx_document(main_topic, research_findings, all_references):
    ...
    The output should be a single, plain text document with no special formatting. Ensure section titles are on their own lines. Do not include a references section. Have the following header in the APA format: Stewart Geisz\n Prof. Jules White\n Software in the age of AI\n Sept 8 2025.
...
def main():
    try:
        ...
        for subcategory in subcategories:
            research_text, _ = research_subcategory(subcategory)
            if research_text:
        ...
        # New: Generate a single list of consolidated references
        all_references = generate_references(main_topic)
